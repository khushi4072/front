    ...
    const rzp1 = new window.Razorpay(options);
      ...
      const saveDataToDatabase = async () => {
        ...
        try {
          ...
          const response = await fetch("http://localhost:5000/saveDataToDatabases", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
            body: JSON.stringify({ name, mail, add, phone, pin ,street,children,adult,checkin, checkout,country,amount,storedTripName}),
          });
...
<Navbar></Navbar>
<form onSubmit={(e) => { e.preventDefault(); checkout1(amount); }}>
